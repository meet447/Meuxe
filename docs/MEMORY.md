...
Every key is optional. `{}` means nothing changed. If the agent omits the block, nothing is written for that turn except the session log and `last_talked_at`; no junk is guessed.
