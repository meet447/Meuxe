...
export function ChatPanel({
  ...
  // ⚡ Bolt: Cache array searches to prevent O(N) traversals on every streaming text update
  const hasRunningTool = useMemo(() => timeline.some(
    (item) => item.kind === "tool" && item.call.status === "running",
  ), [timeline]);

  const hasAnyTool = useMemo(() => timeline.some(
    (item) => item.kind === "tool"
  ), [timeline]);

  // ⚡ Bolt: Memoize the entire timeline mapping to prevent recreating React elements and O(N) traversals
  // on every token when streamingText updates.
