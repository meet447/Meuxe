...
## 2024-05-18 - ChatPanel React List Re-renders\n**Learning:** Mapping over a large array like `timeline` inside the render function without memoization causes O(N) React element recreation and DOM diffing every time the parent component re-renders (e.g. during frequent `streamingText` updates). This is especially costly when generating text token-by-token.\n**Action:** Always wrap heavy list mapping operations like `timeline.map()` in a `useMemo` hook, dependency-keyed to the array reference and necessary callbacks, to avoid O(N) re-renders during high-frequency parent state changes.
